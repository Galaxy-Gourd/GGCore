...
{
    /// <summary>
    /// An instance of a pool - a collection of reusable objects
    /// </summary>
    public abstract class Pool : IPool
    {
        ...
        /// </summary>
        public Pool(DataInputPool data = null)
        {
            if (data != null)
            {
                CapacityMin = data.MinimumCapacity;
                CapacityMax = data.MaximumCapacity;
                SpilloverAllowance = data.SpilloverAllowance;
                PoolLabel = data.Label;
            }
        }
        ...
        public IClientPoolable GetNext()
        {
            ...
            if (_pool.Count > 0)
            {
                ...
                if (result == null)
                {
                    ...
                    {
                        if (SpilloverAllowance == -1 || (SpilloverAllowance > 0 && _pool.Count < _capacityMax + SpilloverAllowance))
                        {
                            result = Spillover();
                        }
                        else
        ...
        void IPool.RelinquishAll()
        {
            for (int i = 0; i < _pool.Count; i++)
            {
                _pool[i].AvailableInPool = true;
                _pool[i].Relinquish();
            }
        }
